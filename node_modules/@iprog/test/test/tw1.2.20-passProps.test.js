...
describe.for([
	...
	it.runIf(shoppingList).for([
	    ...
	    { guests: 5, dishes: [dish2, dish1] },
	])("passes correct ingredients prop to SummaryView", function({guests, dishes, expectedIngredients}, context) {
	    const model = { numberOfGuests: guests, dishes };
	    framework.render(Summary, {model});
	    expect(SummaryView).toHaveBeenCalled();
		const summaryViewProps=SummaryView.mock.lastCall[0];

		if (typeof summaryViewProps.ingredients === "string") {
			context.skip();
		}
